...
/**
 * @see https://github.com/Behat/Transliterator/blob/master/src/Behat/Transliterator/Transliterator.php
 */
class Transliterator
{
    ...
    public static function seemsUtf8($string)
    {
        ...
        for ($i = 0; $i < $stringLength; $i++) {
            ...
            if ((ord($string[$i]) & 0xE0) === 0xC0) { // 110bbbbb
                $n = 1;
    ...
    {
        if (in_array(preg_match('/[\x80-\xff]/', $string), [0, false], true)) {
            return $string;
        }
     ...
     * @param  string  $separator
     */
    private static function postProcessText($text, $separator): string
    {
        ...
        $text = mb_strtolower((string) preg_replace(
            '/[^A-Za-z0-9\/]+/',
            $separator,
            (string) preg_replace(
                '/([a-z\d])([A-Z])/',
